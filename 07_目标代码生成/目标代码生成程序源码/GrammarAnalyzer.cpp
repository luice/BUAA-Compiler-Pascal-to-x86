    ...
    case SymbolType::IDENTIFIER:
        ...
        case IdentifierType::ARRAY:
            ...
            if (SYMBOL_TYPE != SymbolType::LEFT_SQUARE) {
                ...
                NEXTSYM;
            // 默认有左方括号，开始计算表达式
            expressionAnalyze(offset, t, v, false);
            TEST(ErrorEnd::FACTOR);
            // 右方括号
            if (SYMBOL_TYPE != SymbolType::RIGHT_SQUARE) {
                ERROUT(ErrorType::NO_RIGHTSQUARE);
                ...
                NEXTSYM;
            if (addr)
                semanticAnalyzer.getAddress(name, offset, value);
            else {
                string temp;
                semanticAnalyzer.getAddress(name, offset, temp);
                semanticAnalyzer.getContent(temp, value);
            }
            break;
        // 函数调用语句
...
void GrammarAnalyzer::forStatementAnalyze() {
    ...
    // 默认有赋值符，继续
    // 表达式，循环变量的初值
    expressionAnalyze(start, t, v, false);
    TEST(ErrorEnd::FOR_STATEMENT);
    // 给循环变量赋初值
    ...
    // 表达式，循环终止条件
    expressionAnalyze(end, t, v, false);
    TEST(ErrorEnd::FOR_STATEMENT);
    // do 关键字
    if (SYMBOL_TYPE != SymbolType::DO) {
        ERROUT(ErrorType::NO_DO);
